...
/// Read the ygg `200::/7` address from `/proc/net/if_inet6` (musl's `getifaddrs`
/// skips the tun). Each line starts with a 32-hex, colon-less address.
#[cfg(target_os = "linux")]
...
mod tests {
    ...
    fn parses_ygg_address_from_proc_hex() {
        // A /proc/net/if_inet6 with an ygg 200::/7 global (colon-less, 32 hex)
        // followed by ygg0's link-local; the global must be the one picked.
