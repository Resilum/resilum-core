...
fn an_undelivered_message_survives_a_restart() {
    let dir = temp_dir();
    let checkpoint = dir.path().join("lxmf_state");

    let mut node = start("durable", dir.path(), None, Vec::new());
    ...
    let restarted = start("durable", dir.path(), None, Vec::new());
...
fn a_queued_message_keeps_its_id_across_a_restart() {
    ...
    let mut node = start("durable-id", dir.path(), None, Vec::new());
    ...
    let restarted = start("durable-id", dir.path(), None, Vec::new());
